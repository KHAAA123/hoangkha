...
def generate_random_viruss(virus): # tạo hàm radom viruss
    global loop_count
    global bomb_count
    virus_path = "images/" + virus + ".png" #Tạo một đường dẫn đến hình ảnh của vi rútdựa trên tên của vi rútvà đặt nó vào biến 
    data[virus] = {#Tạo một entry mới trong dictionary data với key là virus
        ...
        'throw': False,                         #Ban đầu đặt trạng thái ném ('throw') của vi rútlà False.
        't': 0,                                 #: Khởi tạo giá trị thời gian ('t') là 0
        'hit': False,                           # Ban đầu đặt trạng thái va chạm ('hit') của vi rútlà False.
    }
    if random.random() >= 0.75:     #: Kiểm tra xác suất để quyết định vi rút có nên được ném ra khỏi màn hình hay không. Nếu số ngẫu nhiên lớn hơn hoặc bằng 0.75, 
        #Hàm này trả về một số ngẫu nhiên trong khoảng từ 0 đến 1 (bao gồm 0, nhưng không bao gồm 1).
        data[virus]['throw'] = True #đặt giá trị của thuộc tính 'throw' của vi rút(được định nghĩa trong dictionary data với key là virus) là True. Điều này có thể được hiểu là vi rútsẽ được "ném" ra khỏi màn hình.
    else:
        data[virus]['throw'] = False #Nếu điều kiện sai, đặt giá trị của thuộc tính 'throw' của vi rútlà False. Điều này có thể được hiểu là vi rútsẽ không được "ném" ra khỏi màn hình.
...
#Hiển Thị Quả Cầu trên Màn Hình:
            if value['y'] <= 800: #Nếu quả cầu không rơi ra khỏi màn hình (value['y'] <= 800), hiển thị quả cầu tại vị trí mới.
                gameDisplay.blit(value['img'], (value['x'], value['y']))    #hiển thị vi rútbên trong màn hình một cách linh hoạt
            else:
